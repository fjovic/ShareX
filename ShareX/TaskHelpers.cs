...
namespace ShareX
{
    public static class TaskHelpers
    {
        ...
        public static void ExecuteJob(TaskSettings taskSettings, HotkeyType job, CLICommand command = null)
        {
            ...
            switch (job)
            {
                ...
                case HotkeyType.RectangleLight:
                    new CaptureRegion(RegionCaptureType.Light).Capture(safeTaskSettings, false, true);
                    break;
        ...
        public static void OCRImage(Stream stream, string fileName, string filePath = null)
        {
            if (stream != null)
            {
                using (OCRForm form = new OCRForm(stream, fileName))
                {
                    form.OCRLanguage = Program.Settings.OCRLanguage;
                    form.ShowDialog();
                    Program.Settings.OCRLanguage = form.OCRLanguage;

                    if (!string.IsNullOrEmpty(form.OCRResult) && !string.IsNullOrEmpty(filePath))
                    {
                        string textPath = Path.ChangeExtension(filePath, "txt");
                        File.WriteAllText(textPath, form.OCRResult, Encoding.UTF8);
                    }
                }
            }
        }
